...
local function Main(Shared, Token, Username, Channel, AdminID, RedeemID, CompiledCommandsGrammar)
	...
	local OutputBox = Heartstrings.Mailbox()
	Controller:wrap(
		function()
			while true do
				local Messages = OutputBox:Wait()
				for _, Message in ipairs(Messages) do
					local Connection, Content = Message[1], Message[2]
					...
					if (not Output) then
						...
						if (Event) then
							...
							if CommandPointsToCredit ~= nil then
								...
								OutputBox:Send{
									Connection, Chatter.Command.PRIVMSG(
										"#".. Channel,
							...
							elseif (Event.Command == "PRIVMSG") then
								...
								if Execute then
									...
									if (UserID ~= AdminID) then
										RequiredPoints = CostFunction(Parameters)
										local Total = Shared.Database:GetPoints(UserID)
										if RequiredPoints > Total then
											OutputBox:Send{
												Connection, Chatter.Command.PRIVMSG(Event.Params[1], "You dont have enough Shenanigans points to do that")
											}
											goto continue
										end
									end
									local ResultBox = Heartstrings.Mailbox()
									Execute(Parameters, ResultBox)
									Controller:wrap(
										function()
											local Result = ResultBox:Wait()[1]
											local Success, Error = Result[1], Result[2]
											if not Success then
												OutputBox:Send{Connection, Chatter.Command.PRIVMSG(Event.Params[1], Error)}
											end
										end
									)
								end
